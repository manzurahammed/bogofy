...
function RulesPage() {
	const {
		      perPage, rules, isLoading, error,
		      page, setPage, search, setSearch, statusFilter, setStatusFilter,
		      deleteModal, liveCount, toggleStatus, confirmDelete, openDelete, closeDelete,
	      } = useRulesManager();

	const handleEdit   = useCallback( ( id ) => goToAdmin( `tab=rules&action=edit&rule_id=${id}` ), [] );
	const handleCreate = useCallback( () => goToAdmin( 'tab=rules&action=create' ), [] );
	...
	return (
		<AppShell crumb={CRUMB} actions={actions}>
			<div className="bogo-page-header">
				...
				<RulesToolbar search={search} onSearch={setSearch}
