...
function CreateRulePage() {
	...
	const { step, formData, selectedBuyProducts, selectedFreeProducts, errors, goNext, goPrev, submitRule, isPending } = wizard;

	return (
		<AppShell
			crumb={['Bogofy', __( 'Bogofy Rules', 'bogofy' ), __( 'New rule', 'bogofy' )]}
			actions={
				<WizardActions step={step} onSaveDraft={() => submitRule( 'inactive' )} onNext={goNext}
				               isPending={isPending}/>
			}
		>
			...
			<div className="bogo-wizard">
				<div className="bogo-col" style={{ gap: 16 }}>
					...
					{step < 3 && <WizardNav step={step} onPrev={goPrev} onNext={goNext}/>}
				</div>

				<WizardSidebar formData={formData} step={step}
